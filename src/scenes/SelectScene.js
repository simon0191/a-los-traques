...
export class SelectScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.headerText = this.add.text(GAME_WIDTH / 2, 16, 'ELIGE TU LUCHADOR: JUGADOR 1', {
    ...
    for (let i = 0; i < this.fighters.length; i++) {
      ...
      const nDOM = this.add.dom(cellX + CELL_W / 2, cellY + 40, 'div', {
        ...
        'text-align': 'center', 'width': '44px', 'pointer-events': 'none',
    ...
    this.events.on('shutdown', () => {
      for (const dom of this.portraitDOMs) { dom.destroy(); }
  ...
  updateP1Display() {
    ...
    if (!isRandom && this.anims.exists(`${fighter.id}_idle`)) {
      ...
      this.p1PreviewSprite.setVisible(false); this.p1Portrait.setVisible(true).setFillStyle(parseInt(fighter.color, 16));
  ...
  confirmP1() {
    ...
    if (this.matchContext?.type === 'tournament') {
      ...
      this.time.delayedCall(800, () => {
        ...
        this.cameras.main.once('camerafadeoutcomplete', () => {
          this.scene.start('BracketScene', { gameMode: this.gameMode, matchContext: this.matchContext });
        });
      });
      return;
    }
  ...
  _showP2Selection(idx) {
    ...
    if (!isRandom && this.anims.exists(`${fighter.id}_idle`)) {
      ...
      this.p2PreviewSprite.setVisible(false); this.p2Portrait.setVisible(true).setFillStyle(parseInt(fighter.color, 16));
