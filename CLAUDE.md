...
- **Cloudflare Email Routing**: `infra/email.tf` — per-address forwarder driven by `var.email_forwards` (map of local-part → destination). Email Routing must be activated once via the Cloudflare dashboard (Email → Email Routing → "Get started") because that endpoint's API permission isn't reachable from scoped tokens; activation auto-creates the MX + SPF records. Destinations require email verification before mail actually forwards.
